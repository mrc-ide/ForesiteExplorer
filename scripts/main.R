...
expand_interventions <- function(site_data, expand_year) {
  ridx <- which(site_data$interventions$year == max(site_data$interventions$year))
  ...
  for (i in seq_along(updateridx)) {
    ...
    mean_itn_use <- colMeans(site_data$interventions[start_idx:end_idx, "itn_use"])
...
run_model_for_country <- function(iso, folder_base, net_data, expand_year) {
  ...
  select_years <- max(site_data$prevalence$year)

  site_data <- filter_and_update_interventions(site_data, net_data, select_years)
  site_data <- expand_interventions(site_data, expand_year)
...
if (debug) {
    ...
    iso_codes <- c("NER")
    net_types <- list(PyPyroNets = read.csv(
