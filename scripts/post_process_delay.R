...
create_plot <- function(data, max_year_line, pad_year_line) {
  ggplot(data, aes(x = timestep, y = n_inc_clinical_1825_5474, color = net_types)) +
    ...
    geom_vline(xintercept = pad_year_line, linetype = "dashed", color = "black") +
...
create_plot(combined_data, max_year, max_year + 3)
